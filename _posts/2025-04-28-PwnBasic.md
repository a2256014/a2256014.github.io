...
3. GOT Leak
write 함수를 통해 read GOT 값을 출력시켜 주소 Leak
```Python
# write(1, read_got, ...)
...
4. GOT Overwrite
read 함수의 GOT를 system 주소로 변조한 뒤 read함수 호출
```Python
...
5. system 호출
read_got에 `system 주소 + "/bin/sh\x00"` 로 총 16byte를 입력하였기 때문에 `"/bin/sh\x00"`는 read_got+0x8 주소에 
```Python
