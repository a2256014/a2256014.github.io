...
![](/assets/images/posts/2025-04-28-PwnBasic/33544fdc81620dd5361682489c8b61f6_MD5.jpeg)
...
| `flags`     | 3비트    | 64비트 환경에서 청크는 16바이트 단위로 할당되므로, `size`의 하위 4비트는 의미를 갖지 않습니다. 그래서 ptmalloc은 `size`의 하위 3비트를 청크 관리에 필요한 플래그 값으로 사용합니다.<br><br>각 플래그는 순서대로 allocated arena(A), mmap’d(M), prev-in-use(P)를 나타낸다. `prev-in-use` 플래그는 직전 청크가 사용 중인지를 나타내므로, ptmalloc은 이 플래그를 참조하여 병합이 필요한지 판단할 수 있습니다. _나머지 플래그에 대해서는 여기서 설명하지 않겠습니다._ |
