...
>에뮬레이션으로 진행 시 설정한 포트로 디버깅하려 한다면, 명령어에 `hostfwd` 옵션을 통해 해당 포트를 포워딩을 해주어야 합니다.
...
#### 공격 벡터 찾기
아래와 같은 동작들을 기반으로 공격 벡터를 탐지해보자.
##### 사용자 입력 부분
예를 들어 USB 포트가 있는 경우, 이 또한 하나의 입력으로써 USB Hot plugging을 담당하는 프로그램(e.g. mdev)의 취약점 분석을 진행해도 되고 혹은 gdbserver를 USB를 통해 옮길 수 있다.

이외에도 네트워크 통신, 하드웨어 인터페이스 통신 등의 입력이 있다.
##### 실행 중인 프로세스
>실행 중인 프로세스 이외에도 보통은 실행되지 않다가 특정 시점에 실행되는 프로그램도 존재한다. `dream_dict` 바이너리가 이와 같이 실행된다.

만약 사용자 입력은 A 프로세스에서 받는데, B 프로세스에서 해당 입력 값을 처리한다면 A 프로세스만 분석해서는 안되고 B 프로세스도 분석해야 한다.

또한 오픈소스 프로그램이 작동 중일 경우 1 day 취약점을 시도하자
##### 기기의 설정

- root 계정의 비밀번호를 설정하는 명령어가 담겨있음 → root 계정의 평문 비밀번호 탈취 가능.
- telnetd가 파일 시스템에 존재하지만, 초기 설정에서는 실행이 되지 않음 → RCE 성공시 telnetd 실행하여 bind 셸 실행 가능.
- 웹 사이트 관리자 계정이 파일 시스템 내에 평문으로 저장되어 있음 → 파일을 읽어 관리자 계정 탈취 가능.
