...
```shell
# 스캔할 IP 방화벽 인열
crackmapexec smb 192.168.50.242 -u <user> -p "password" --shares
```
### SMTP Enum
```shell
# user 확인
...
#### Pishing Mail
``` shell
sudo swaks -t <수신자> -t marcus@beyond.com --from john@beyond.com --attach @config.Library-ms --server 192.168.50.242 --body @body.txt --header "Subject: Staging Script" --suppress-data -ap
```