...
대충은 라즈베리파이[연결보드]와 타겟 보드의 JTAG 모든 핀을 연결하고, JTAGEnum.sh 스크립트를 이용해서 핀 정보를 파악한다. 이때, Loopback이 발생하는 핀이 있으면 제거하고 Scan 진행한다. 스크립트를 돌리면 어떤 핀이 JTAG 핀인지 알 수 있고, 이제 얻은 정보를 토대로 J-Link[디버거]와 연결을 진행한다. 그 후에 OpenOCD
